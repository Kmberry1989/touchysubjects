...
test('compile smoke: all include dependencies resolve to model or vendor path', () => {
  for (const entry of catalog.entries) {
    for (const dep of entry.includeDeps) {
      const modelCandidate = path.join(modelRoot, path.basename(dep));
      const vendorCandidate = dep.replace(/^\.\//, '');
      const ok = fs.existsSync(modelCandidate) || vendorFiles.has(vendorCandidate);
